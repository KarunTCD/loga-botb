...
namespace LoGa.LudoEngine.Services
{
    public class FirebaseService : MonoBehaviour, IFirebaseService
    {
        ...
        public async Task TestWriteAndRead()
        {
            if (dbReference == null)
            {
                Debug.LogError("Firebase not initialized — dbReference is null");
                return;
            }
            ...
            {
                // Write a test value
                await dbReference.Child("testNode").SetValueAsync("hello");

                // Read it back
                var snapshot = await dbReference.Child("testNode").GetValueAsync();

                if (snapshot.Exists)
                    Debug.Log($"[FirebaseService] Test read success: {snapshot.Value}");
                else
                    Debug.LogWarning("[FirebaseService] Test node does not exist after write.");
            }
            catch (Exception e)
            {
                Debug.LogError($"[FirebaseService] Test write/read failed: {e.Message}");
            }
        }
